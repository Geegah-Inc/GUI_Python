...
pos201 = ax2.imshow(im2, vmin = -0.15, vmax = 0.15, cmap = 'ocean', interpolation = 'gaussian')
...
for cf in range(numFrames):
    ...
    Q_del2 = F_IMG_Q  - AIR_Q
    #update title to reflect frame count
    mytitle.set_text(base_title+str(cf)+' of ' +str(numDigits-1))
    
    sub_image = np.flipud(np.rot90(Phase_S - Phase_A,1))
    sub_image = np.rot90(sub_image,3)
    
    pos201.set_data(sub_image)
    pos201.set_clim(-0.01,0.01)
